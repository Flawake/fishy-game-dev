...
Output is a plain-field DTO shape that both Unity's JsonUtility and
Newtonsoft.Json read identically:
...
Bodies are serialised with Newtonsoft rather than JsonUtility, because
JsonUtility has no concept of null for a nested [Serializable] class: it writes
a default-constructed object in place of a null one, and reads a null one back
as a default-constructed object. An optional nested schema (`durability`,
`stack`, `reward_item`, ...) therefore cannot survive a JsonUtility round trip
-- absent becomes present-and-zero in both directions.
"""
        ...
        for i, o in enumerate(sorted(by_tag[tag], key=lambda x: x["method"])):
            ...
            if o["req"]:
                ...
                out.append(f'            ApiDispatch.Send("{o["verb"]}", "{o["path"]}", '
                           f'ApiJson.Serialize(body), {o["parser"]}, onComplete);')
            else:
